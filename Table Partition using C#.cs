...
    partition = partition ?? data?.partition;
    sourceschema = sourceschema ?? data?.sourceschema;
...
    Table t = m.Tables[table];
    ...
    int minyear= currentYear-int.Parse(history);
...
            	            Query = @"SELECT * FROM " + sourceschema + "." + sourceobject + " WHERE [Is Imported]='Y' and YEAR([Invoice Date]) >= "+ minyear.ToString(),
                	    }
	                }
    		    );
...
    //creating yearly partition if not exist in the Table till year before previous year
    ...
    for (int year=minyear;year<previousyear;year++)
    {   
...
                        {
                            DataSource = m.DataSources[datasource],
                            Query = @"SELECT * FROM " +sourceschema+"." +sourceobject+ " WHERE [Is Imported]='N' and YEAR([Invoice Date]) ="+year,
                        }
                	}
        		);
...
                    	{
                        	DataSource = m.DataSources[datasource],
                        	Query = @"SELECT * FROM " +sourceschema+"."+ sourceobject +  " WHERE [Is Imported]='N' and YEAR([Invoice Date]) ="+previousyear,
                    	}
                	}
        		);
    ...
    /////
/* this will create partition for last year if we are executing this before april
...
                	new Partition()
                	{
                    	Name = previousyear+"0"+i.ToString(),
                    	Source = new QueryPartitionSource()
                    	{
                        	DataSource = m.DataSources[datasource],
                        	Query = @"SELECT * FROM " +sourceschema+"." + sourceobject +  " WHERE [Is Imported]='N' and YEAR([Invoice Date]) ="+previousyear +" and Month([Invoice Date])="+i,
                    	}
                	}
        		);
...
                    	{
                        	DataSource = m.DataSources[datasource],
                        	Query = @"SELECT * FROM " +sourceschema+"." + sourceobject +  " WHERE [Is Imported]='N' and YEAR([Invoice Date]) ="+currentYear +" and Month([Invoice Date])="+i,
                    	}
                	}
        		);
...
                    	{
                        	DataSource = m.DataSources[datasource],
                        	Query = @"SELECT * FROM " +sourceschema+"." + sourceobject + " WHERE [Is Imported]='N' and YEAR([Invoice Date]) ="+nextyear +" and Month([Invoice Date])="+i,
                    	}
                	}
        		);
